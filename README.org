...
なお、keychain のパスフレーズの入力を回避する方法としては、ssh-agent-wsl を使う方法があるよう
です。詳しくは、以下のサイトを参照してください。
...
wslclient.exe に引数を指定しない場合でも emacs のフレームを開くようにしています。wslclient.exe のショートカット
をデスクトップ等に置いておけば、ダブルクリックで emacs のフレームを開くことができます。
